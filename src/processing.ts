...
import type { FaceDetectorOptions, HandDetectorOptions, IModel, ObjectDetectorOptions, Options, PoseDetectorOptions } from './types.js';
  ...
  async start(video: HTMLVideoElement) {
    ...
    switch (this.#currentMode) {
      case `pose`: {
        ...
        this.#model = new PoseDetector(this.poseOptions);
        break;
      }
      case `objects`: {
        this.log.info(this.objectDetectorOptions);
        this.#model = new ObjectDetector(this.objectDetectorOptions);
        break;
      }
      case `face`: {
        this.log.info(this.faceDetectorOptions);
        this.#model = new FaceDetector(this.faceDetectorOptions);
        break;
      }
      case `hand`: {
        this.log.info(this.handDetectorOptions);
        this.#model = new HandDetector(this.handDetectorOptions);
        break;
      }
      default: {
